...
const RECEIPT_LOG = new Map<string, { ts: number; receipt: unknown }>();
...
function stamp(nonce: string, receipt: unknown): void {
  RECEIPT_LOG.set(nonce, { ts: Date.now(), receipt });
}

export function sendOkxV2Challenge(
  reply: FastifyReply,
  opts: {
    resource: string;
    description: string;
  ...
  if (challenge.accepts[0]) challenge.accepts[0].asset = USDT0_XLAYER;
...
export function verifyOkxV2Receipt(
  sigHeader: string | undefined,
  expected: { amount: string; payTo: string; network?: string }
): { ok: true; receipt: any; format: 'okx-v2' } | { ok: false; reason: string } {
  if (!sigHeader) {
...
export function requireOkxV2Payment(
  req: FastifyRequest,
  reply: FastifyReply,
  priceUsd: number
): { ok: true } | null {
  const sig = (req.headers['payment-signature'] || req.headers['x-payment']) as string | undefined;
  if (!sig) {
