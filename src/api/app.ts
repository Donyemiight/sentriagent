...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  // ─── Assess Wallet ──────────────────────────────────────────
  app.post<{ Body: { chain: string; address: string } }>('/v1/assess-wallet', async (req, reply) => {
    const ok = await requirePayment(req, reply, config.pricePerCall);
    if (!ok) return;
    ...
    const verdict = await assessWallet({ chain: chain as any, address });
    return verdict;
  });

  // ─── Assess Transaction ─────────────────────────────────────
  app.post<{ Body: { chain: string; from: string; to: string; data?: string; value?: string } }>(
    '/v1/assess-tx',
    async (req, reply) => {
      ...
      const verdict = await assessTx({ chain: chain as any, from, to, data, value });
      return verdict;
    }
  );

  // ─── Bundle Assess (5 tokens in 1 call) ─────────────────────
  app.post<{ Body: { tokens: Array<{ chain: string; address: string }> } }>(
    '/v1/bundle-assess',
    async (req, reply) => {
      ...
      const result = await bundleAssess(tokens as any);
      return result;
    }
  );

  // ─── Payment verify endpoint ────────────────────────────────
  app.post<{ Body: { paymentId: string; proof: string; txHash?: string } }>(
    '/v1/payment/verify',
    async (req, reply) => {
