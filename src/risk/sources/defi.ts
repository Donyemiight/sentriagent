...
export async function fetchDeFiSignal(
  chain: string,
  address: string
): Promise<DeFiSignal> {
  ...
  try {
    ...
    // Score fusion
    let contribution = 0;
    if (sanctioned) contribution -= 50; // fatal
    if (rugHistory) contribution -= 30;
    if (exploitExposure) contribution -= 20;

    if (ageInDays !== undefined) {
      if (ageInDays > 365) contribution += 10; // > 1yr old = trusted
      else if (ageInDays > 90) contribution += 5;
      else if (ageInDays < 7) contribution -= 10; // brand new = risky
    }

    if (txCount !== undefined && txCount > 100) contribution += 5;
