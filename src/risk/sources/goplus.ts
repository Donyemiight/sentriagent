...
export async function fetchGoPlusSignal(
  chain: string,
  address: string
): Promise<GoPlusSignal> {
  ...
  try {
    ...
    if (isHoneypot) contribution -= 50; // fatal
    if (buyTax > 10 || sellTax > 10) contribution -= 20;
    else if (buyTax > 5 || sellTax > 5) contribution -= 10;
    if (transferTax > 5) contribution -= 10;
    if (canBlacklist) contribution -= 15;
    if (canSelfDestruct) contribution -= 30;
    if (isUpgradeable) contribution -= 10;
