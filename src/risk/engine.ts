...
import { fetchOkxTokenSignal, type OkxTokenSignal } from './sources/okx.js';
import { fetchGoPlusSignal, type GoPlusSignal } from './sources/goplus.js';
import { fetchDeFiSignal, type DeFiSignal } from './sources/defi.js';
 ...
 */
export async function assessToken(input: AssessTokenInput): Promise<RiskVerdict> {
  ...
  for (const [name, result] of [['okx', okx], ['goplus', goplus], ['defi', defi]] as const) {
  ...
  if (signals.goplus) {
    ...
    if (signals.goplus.isHoneypot) honeypotFlag = true;
    sources.push('GoPlus Security');
  }
  if (signals.defi) {
    score += signals.defi.scoreContribution;
    if (signals.defi.rugHistory) rugHistoryFlag = true;
    sources.push('De.Fi');
  }

  // Hard overrides — these are non-negotiable
  if (honeypotFlag) score = Math.min(score, 10);
  if (rugHistoryFlag) score = Math.min(score, 15);
 ...
 */
export async function assessWallet(input: AssessWalletInput): Promise<RiskVerdict> {
  const start = Date.now();
  ...
  if (signals.goplus) {
    score += signals.goplus.scoreContribution;
    if (signals.goplus.isHoneypot) score = Math.min(score, 10);
    sources.push('GoPlus Security');
  }
  if (signals.defi) {
    score += signals.defi.scoreContribution;
    if (signals.defi.rugHistory) score = Math.min(score, 15);
    sources.push('De.Fi');
  }
 ...
 */
export async function assessTx(input: AssessTxInput): Promise<RiskVerdict> {
  const start = Date.now();
 ...
 */
export async function bundleAssess(
  tokens: Array<{ chain: AssessTokenInput['chain']; address: string }>
): Promise<{ verdicts: RiskVerdict[]; avgScore: number; allSafe: boolean }> {
...
function buildRecommendation(score: number, honeypot: boolean, rugHistory: boolean): string {
  if (honeypot) return 'BLOCK: Honeypot detected. Do not transact.';
  if (rugHistory) return 'BLOCK: Address has historical rug patterns. Do not transact.';
  if (score >= 90) return 'PROCEED: Established asset with strong signals.';
  if (score >= 70) return 'PROCEED: Low risk, normal precautions apply.';
  if (score >= 50) return 'CAUTION: Medium risk. Limit position size, verify manually.';
  if (score >= 30) return 'HIGH RISK: Multiple warning flags. Recommend skipping this asset.';
