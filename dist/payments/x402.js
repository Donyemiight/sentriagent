 ...
 */
export function sendPaymentChallenge(req, reply, priceUsd, intent = 'charge') {
 ...
 */
export async function verifyPayment(paymentId, proof, txHash) {
    if (!paymentId || !proof) {
 ...
 */
export async function requirePayment(req, reply, priceUsd) {
    // OKX x402 v2 path: check PAYMENT-SIGNATURE first (marketplace standard)
    const okxSig = (req.headers['payment-signature'] || req.headers['x-payment']);
    if (okxSig) {
